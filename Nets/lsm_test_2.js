signal_file = "/home/nico/neuro/signal_unverzerrt.txt";
time_offset = 10;
grid_x = 5;
grid_y = 5;
input_synapses = 1;
...
evaluate = function evaluate(){
	...
	while(sim.time < 1600){
		...
		if(bit_time > 0){
			...
			x = output.value() - target;
			sum += x*x;
			count++;
		}
	}
	output.save_samples_to_file("learned_output.nsamples");
	write_to_file("rms_output.txt", Math.sqrt(sum/count));
}
