...
void Group::do_backprop(double target_value, LinearDiscriminator* output_neuron){
    ...
    Log::instance().log( "==Doing backpropagation run==", this, Log::DEBUG_MSG );
    Log::instance().log( QString("Backprop: target=%3, error(%1)=%2").arg(output_neuron->objectName()).arg(errors[output_neuron]).arg(target_value).toStdString(), this, Log::DEBUG_MSG );
    ...
    while(!current_layer.empty()){
        ...
        BOOST_FOREACH(LinearDiscriminator* ld, current_layer){
            BOOST_FOREACH(LDConnection* conn, ld->inputs()){
                ...
                Log::instance().log( QString("Backprop: delta_w(%1,%2): %3 + %4 = %5").arg(conn->pre_neuron()->objectName()).arg(conn->post_neuron()->objectName()).arg(conn->weight()).arg(delta_w).arg(conn->weight() + delta_w).toStdString(), this, Log::DEBUG_MSG );
